...
modules.exports = function(creep){
    if((creep.store.energy != 0 && creep.memory.status == 'repair') || (creep.store.getFreeCapacity(RESOURCE_ENERGY) == 0 && creep.memory.status == 'get')){
        creep.memory.status = 'repair';
        var repairSite_id = creep.memory.task.target_id;
        var repairSite = Game.getObjectById(repairSite_id);
        if(repairSite.hits >= repairSite.hitsMax*0.9 || ((repairSite.structureType == STRUCTURE_WALL || repairSite.structureType == STRUCTURE_RAMPART) && repairSite.hits >= 12000)){
